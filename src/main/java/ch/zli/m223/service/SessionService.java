...
public class SessionService {
    ...
    public Response authenticate(Credential credential) {
        Optional<ApplicationUser> principal = applicationUserService.findEmail(credential.getEmail());

        try {
            if (principal.isPresent() && principal.get().getPassword().equals(credential.getPassword())) {
                String token = Jwt.issuer("https://myspace.ch/")
                ...
                return Response
                        .ok(principal.get())
                        .cookie(new NewCookie("myspace", token))
